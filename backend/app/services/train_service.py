...
class TrainService:
    ...
    async def create_train(
        ...
        existing_train = await self.repository.get_by_train_number(
        ...
        train = Train(
            ...
            total_coaches=train_data.total_coaches,
            status=train_data.status,
        )

        return await self.repository.create(train)
    ...
    async def update_train(
        ...
        if (
            ...
            and update_data["train_number"] != train.train_number
        ):
            existing_train = await self.repository.get_by_train_number(
        ...
        return await self.repository.update(train)
